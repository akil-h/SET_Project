...
liwc <- read_excel("Course_Evaluation_Summary_Sheet.xlsx")

liwc$`Comment 7` <- enc2utf8(as.character(liwc$`Comment 7`)) #UTF-8 encoding
liwc$`Comment 8` <- enc2utf8(as.character(liwc$`Comment 8`)) #UTF-8 encoding

liwc$`Comment 7` = tolower(liwc$`Comment 7`) #transform to lower case
liwc$`Comment 8` = tolower(liwc$`Comment 8`) #transform to lower case

#8: Lemmatize: if you skipped this step before, skip it here as well.

toks = VCorpus(VectorSource(liwc$`Comment 7`))
...
#9: Match target text to dictionaries & convert to document-feature data; bind data.         STUCK HERE AND ABOVE
...
toks_dict$valuex = SADCAT::clean_large_text(liwc$'Comment 7')
